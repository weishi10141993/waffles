...
try:
    ...
    print(
        ...
        "Do not use 'pyroot' library options."
    )
...
def WaveformSet_from_ROOT_files(
    ...
    if len(valid_filepaths) == 0:
        raise Exception(
            generate_exception_message(
                2,
                'WaveformSet_from_ROOT_files()',
                "No valid ROOT files were found in the given folder or filepath list."
            )
        )
    if verbose:

        print(
            f"In function WaveformSet_from_ROOT_files(): Found {
        ...
        print("\n", end='')
        print(
            f"In function WaveformSet_from_ROOT_files(): Reading file 1/{
                len(valid_filepaths)} ..."
        )
    ...
    for filepath in valid_filepaths[1:]:
        ...
        if verbose:
            print(
                f"In function WaveformSet_from_ROOT_files(): Reading file {
    ...
    if verbose:
        print("In function WaveformSet_from_ROOT_files(): Reading finished")
...
def WaveformSet_from_ROOT_file(
    ...
    # One could consider summing wf_start to every entry of
    # aux at this point, so that the __build... helper
    # functions do not need to take both parameters idcs_to_retrieve
    # and first_wf_index. However, for
    # the library == 'uproot' case, it is more efficient to clusterize
    # first (which is done within the
    # helper function for the uproot case), then sum wf_start.
