...
class BeamWfAna(WfAna):
    """Stands for Beam Waveform Analysis. This 
    ...
    def analyse(self, waveform: WaveformAdcs) -> None:
        ...
        self._WfAna__result = WfAnaResult(
            ...
            baseline_min=np.min(baseline_samples),
            # np.max(baseline_samples),
            baseline_max=np.max(baseline_samples),
            # and ~np.std(baseline_samples))
            baseline_rms=np.std(baseline_samples),
